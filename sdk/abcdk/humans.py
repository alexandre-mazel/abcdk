...
class Human(dict):
    ...
    def __init__( self, nID, strLastName = "Unknown", strChristianName = "Unknown", strCountry = "fr", dictPronounciation = {} ):
        dict.__init__(self)
        self.nID = nID;
        self.nRecoID = -1; # Face Recognition id related to this Human
        ...
        self.likes = {} # for each attribute, the human liking. --  eg: "green beans": 10. 10: like very much, 0: dislike, 5: average
    # __init__ - end
...
class HumanManager:
    ...
    def enforceHumanExists( self, variantHumanDesc, nId = -1 ):
        """
        ...
        - nId: optionnal id (else extract from variantHumanDesc)            
        """
        print( "INF: enforceHumanExists: receiving: '%s'" % variantHumanDesc )
        if( typetools.isString( variantHumanDesc ) ):
            print( "WRN: HumanManager.enforceHumanExists(%s): is string !" % variantHumanDesc )
            ...
            print( "WRN: HumanManager.enforceHumanExists(%s): num human: %s, strName: %s, christianname: %s" % (variantHumanDesc,nNumHuman,strName, strChristianName) )
        else:
            nNumHuman = int( variantHumanDesc ); # adding int(just to be sure)
            strName, strChristianName = ["Smith", "Jean"]
            
        if nId == -1:
            nId = nNumHuman
        
        print( "DBG: HumanManager.enforceHumanExists(%s): current keys: %s" % (variantHumanDesc, self.dictHumans.keys() ) )
        if nId in self.dictHumans.keys():
            return
                
        print( "WRN: HumanManager.enforceHumanExists(%s) => creating a new human on the fly (name: %s, surname: %s)" % (variantHumanDesc,strName, strChristianName) );
        nNumHuman = self.getHumanID( variantHumanDesc );
        self.dictHumans[nId] = Human(nNumHuman)
        self.dictHumans[nId].strLastName = strName
        self.dictHumans[nId].strChristianName =  strChristianName
        
        print( "DBG: HumanManager.enforceHumanExists(%s): at end: current keys: %s" % (variantHumanDesc, self.dictHumans.keys() ) )
    ...
    def getHumanInfo( self, variantHumanDesc ):
        ...
        try:
            ...
            print( "WRN: HumanManager.getHumanInfo(%s): key err: %s" % ( str(variantHumanDesc), err ) );
            self.enforceHumanExists(variantHumanDesc)
            self.dictHumans[nNumHuman];            
        return None;
    # getHumanInfo - end
    ...
    def setLangAndGetInfo( self, variantHumanDesc, bDontSwitchLang = False ):
        ...
        try:
            ...
            try:
                ...
                print( "ERR: HumanManager.setLangAndGetInfo: variantHumanDesc: %s =>, err: %s" % (str( variantHumanDesc ),str(err)) );
            ...
            return [bIsKnown, pronounce[0], pronounce[1], strCountry, timeLastSeen, nCptTimeSeen];
                ...
                print( "ERR: HumanManager.setLangAndGetInfo (2): variantHumanDesc: %s =>, err: %s" % (str( variantHumanDesc ),str(err)) );
