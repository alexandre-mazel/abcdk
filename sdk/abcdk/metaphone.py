...
# Alma: taken from https://github.com/dracos/double-metaphone/blob/master/metaphone.py
...
def getTextSeparators():
    "chars frequently found in text that are not part of words"
    return " ,.;:!?/()"

def stripUnicode( su, listChars = " " ):
    # strip beginning
    while( len(su) > 0 and su[0] in listChars ):
        su = su[1:]
    while( len(su) > 0 and su[-1] in listChars ):
        su = su[:-1]
    return su
...
def trimNonMeta( s ):
    """
    trim from a string any non audible letter
    eg: "flageolet?" => "flageolet" 
    """
    
    # at begin
    while(1):
        print ("'%s'" % s )
        if( len(s) > 0 and dm(unicode(s[1:])) != dm(unicode(s)) ):
            print( dm(unicode(s[1:]) ) )
            print( dm(unicode(s) ) )
            break
        s = s[1:]
...
def fromMetaphoneAtEnd( meta, hint ):
    """
    ...
    if 0:
        ...
        for i in range(len(hint)-1, -1, -1 ):
            submeta = dm(unicode(hint[i:]))
            #~ print( "submeta: %s" % submeta[0] )
            if( submeta[0] == meta ):
                return trimNonMeta(hint[i:])
    ...
    if 1:
        ...
        for i in range(len(hint)-1, -1, -1 ):
            if( hint[i] in " ,.;:!?" ):
                submeta = dm(unicode(hint[i:]))
                print( "submeta: %s" % submeta[0] )
                if( submeta[0] == meta or meta in submeta[0] ): # equal or the first describing the word or more of it                    
...
def autoTest():
    assert_check( dm( unicode("apples") ), ('APLS', '') )
    assert_check( dm( unicode(" apples") ), ('APLS', '') )
    assert_check( dm( unicode("checkin please!") ), ('XKNPLS', '') )
    assert_check( dm( unicode("checking please!") ), ('XKNKPLS', '') )
    assert_check( dm( unicode("chicken please!") ), ('XKNPLS', '') )
    
    
    assert_check( trimNonMeta( " les flageolet?" ), "les flageolet" )
    assert_check( trimNonMeta( " apples" ), "apples" )
    
    assert_check( dm( unicode("tu aimes les pommes?") ), ('TMSLSPMS', '') )
    assert_check( dm( unicode("les flageolet?") ), ('LSFLJLT', 'LSFLKLT') )
    assert_check( dm( unicode("les flageolets?") ), ('LSFLJLTS', 'LSFLKLTS') )
    assert_check( dm( unicode("Do you like apples?") ), ('TLKPLS', '') )
    assert_check( dm( unicode("Did you like cucumbers?") ), ('TTLKKKMPRS', '') )
    assert_check( dm( unicode("apples") ), ('APLS', '') )
    ...
    assert_check( fromMetaphoneAtEnd( "LSFMS", "est-ce que tu aimes les femmes ?" ), "les femmes" )
    #~ assert_check( fromMetaphoneAtEnd( "PLS", "Do you like apples?" ), "ples" ) # current problem: the a is mixed with the end of the like "likaplle"
