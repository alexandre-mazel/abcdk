...
import abcdk.sound_old as sound_old
...
global_bUsageNoiseExtractorIsPresent = False; # cpu optim: if no analyseSound present at first call, then disable test
def analyseSound_pause( bWaitForResume ):
    ...
    print( "WRN:please port this method again (abcdk/split/JM breakage)")
    global global_bUsageNoiseExtractorIsNotPresent;
    if( not global_bUsageNoiseExtractorIsPresent ):
        return;    
    try:
...
def playSound( strFilename, bWait = True, bDirectPlay = False, nSoundVolume = 100 , bUseLedEars=True):
    ...
    try:
        ...
        sound_old.analyseSound_resume( bWait )
...
def stopSound( strFilename = None, bDirectPlay = False ):
    ...
    if( not bDirectPlay ):
        ...
        if( ap != None ):
            ...
            sound_old.analyseSound_resume( True )
            return
    os.system( "killall aplay" )
    sound_old.analyseSound_resume( True ) # TODO: kill the right sound !
# stopSound - end
...
def setInputVolume( nVolPercent ):
    ...
    ledsproxy = naoqitools.myGetProxy( "ALLeds" )
    ledsproxy.setIntensity( "EarLeds", nVolPercent / 100. )
# setInputVolume - end
