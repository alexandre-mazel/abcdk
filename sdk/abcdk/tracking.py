...
class TrackHuman:
    ...
    def __init__( self ):
        ...
        self.sl = None;
        self.id = -1;
        self.timeBegin = time.time() # we add an offset for posting in the ALMemory, because we exploded recently the float precision
        ...
        if( system.isOnOtherRobotThanNao() ):
            self.debug( " INF: abcdk.TrackHuman.__init__: We are on another robot than NAO" );
            self.nSharpnessMethod = 3;   
            self.nSharpnessValue = 2;
            self.rSharpnessMultiplier = 1.;
            self.rSharpnessRatioWindow = 0.4;
        else:
            self.debug( " INF: abcdk.TrackHuman.__init__: We are on NAO" );
        self.debug( " INF: abcdk.TrackHuman.__init__: rSharpnessMultiplier: %f" % self.rSharpnessMultiplier );
        ...
        if( system.isOnRomeo() ):
            self.debug( " INF: abcdk.TrackHuman.__init__: We are on Romeo!" );
            self.bTrackWithEyes = True;
            self.bUseTrunk = True;
            self.rSpeedTrackX = 0.1;
            self.rSpeedTrackY = 0.2;
            self.rAmortiHeadX = 0.5;
            #self.eyesMover = motiontools.EyesMover();
            self.eyesMover = motiontools.eyesMover;
            self.debug( " INF: abcdk.TrackHuman.__init__: using eyesmover: %s!" % self.eyesMover );
            ...
            self.rPitchRatioThruJoint = [0.65, 0.35]
            import romeo
        else:
            self.rSpeedTrackX = 0.05;
            self.rSpeedTrackY = 0.1;
            self.rAmortiHeadX = 0.8
            self.bUseTrunk = False;
            self.strLauncherName = "ALLauncher";            
            self.rThresholdToMove = 0.05;
            self.headChain = ["HeadYaw","HeadPitch"];
            self.strMemoryVideoName = "ALMemory";
            self.rPitchRatioThruJoint = [1.]
        ...
        if( not self.bUseNativeExtractor ):
            try:
                ...
                if( self.nComplainAboutNotHavingPause < 10 ):
                    ...
                    self.debug( "DBG: should be a normal error: '%s'" % str( err ) );
                pass
    ...
    def prepare( self, bState = True ):
        ...
        while( self.mutex.testandset() == False ):
            self.debug( "INF: abcdk.TrackHuman.prepare: already preparing, waiting..." );
            time.sleep( 0.1 );
        ...
        if( bState ):
            try:
                if( self.bUseNativeExtractor ):
                    ...
                    if( self.extractor == None ):
                        self.debug( "INF: abcdk.TrackHuman.prepare: creating proxy to HaarExtractor..." );
                    ...
                    if( self.fr == None ):
                        ...
                        if( self.fr == None ):
                            self.debug( "WRN: abcdk.TrackHuman.prepare: no face recognition module 'ALFaceRecognition' found, you couldn't use that feature..." );
                ...
                try:
                    ...
                    self.debug( "WRN: abcdk.TrackHuman.prepare: subscribe fail (possible on romeo, on second time...) $$$, err: %s" % err );
            ...
            if( self.extractor != None ):
                ...
                if( system.isOnRomeo() and False ): # now it works!
                    self.debug( "WRN: abcdk.TrackHuman.prepare: unsubscribing from extractor - generate a failure, won't unsubscribe !!! TODO WARNING $$$$" );
                else:
                    self.debug( "INF: abcdk.TrackHuman.prepare: unsubscribing from extractor - begin" );
                    self.extractor.unsubscribe( self.getName() );
                    self.debug( "INF: abcdk.TrackHuman.prepare: unsubscribing from extractor - end" );
                
            if( self.bSoundLocalisationIsRunning ):
                self.bSoundLocalisationIsRunning = False;            
                self.debug( "DBG: abcdk.TrackHuman.prepare: SoundLocalization - stop" );           
            ...
            if( self.id != -1 ):
                try:
                    ...
                    self.debug( "DBG: abcdk.TrackHuman.update: normal error: %s" % str( err ) );
                    pass
                self.id = -1;     
        ...
        self.idY = -1;
        self.idTrunk = -1;
        self.abLastFrameFastMove = [False,False]; # [on axis X, on axisY]: if last frame has a big move, we skip the new face position (as it's very unprecise) TODO: have the camera position relative to the analysed image!
    ...
    def start( self, rMaxDistance = 1.5 ):
        ...
        self.debug( "INF: abcdk.TrackHuman.start: dist: %f => minsize: %d" % ( rMaxDistance, nMinSize ) );
        if( self.extractor != None ):
            if( not self.bUseNativeExtractor ):
                self.extractor.setMinSize( nMinSize, nMinSize );
        try:
            self.extractor.setFrameRate( 30 );
        except BaseException as err:
            self.debug( "WRN: abcdk.TrackHuman.start: error in setFrameRate: %s" % ( err ) );    
            pass
        try:
            self.extractor.setSaveImageWithFace(1)
        except BaseException as err:
            self.debug( "WRN: abcdk.TrackHuman.start: error setSaveImageWithFace isn't implemented? err: %s" % ( err ) );
    ...
    def stopAllMoves(self):
        ...
        if( self.id != -1 ):
            try:
                self.debug( "DBG: stopAllMoves: stopping move task id: %d" % ( self.id ) );
                self.motion.stop( self.id );
            except BaseException, err:
                self.debug( "DBG: abcdk.TrackHuman.update: normal error: %s" % str( err ) );
                pass
        ...
        if( self.idX != -1 ):
            try:
                self.debug( "DBG: stopAllMoves: stopping move task idX: %d" % ( self.idX ) );
        ...
        if( self.idY != -1 ):
            try:
                self.debug( "DBG: stopAllMoves: stopping move task idY: %d" % ( self.idY ) );                
        ...
        if( self.idTrunk != -1 ):
            try:
                self.debug( "DBG: stopAllMoves: stopping move task idTrunk: %d" % ( self.idTrunk ) );                                
    ...
    def pause( self, bState = True ):
        ...
        """
        while( self.mutex.testandset() == False ):
            self.debug( "INF: abcdk.TrackHuman.pause: already pausing, waiting..." );
            time.sleep( 0.1 );        
        if( bState ):
            self.nNbrPaused += 1;
        else:
            if( self.nNbrPaused == 0 ):
                self.debug( "WRN: abcdk.TrackHuman.pause(False): already unpaused" );
                self.mutex.unlock();
    ...
    def getFaceVariableMemoryName(self):
        return  ["Tracking/NoFace", "Tracking/FaceSeen", "Tracking/FaceAnalysed","Tracking/FaceRecognised"];
    ...
    def update( self, faceInfos, bTrackFace = True, bLaunchFaceRecognition = False, rFaceRecognitionThreshold = 0.82, bInhibateRecognisedWhenHeadHasMovedSinceAcquisition = True, bAnimateLed = True, bUseSoundLocalisation = False, bEnableRandomMoves = False ):
        ...
        return 
            ...
            raise also info in:
                - Tracking/NoFace: analyse doesn't find any face
        ...
        """
        #~ self.debug( "ENTERING UPDATE   " * 10 );
        if( faceInfos != [] ):
            self.debug( "\n\nDBG: INF: abcdk.TrackHuman.update: in with faceInfos: %s" % str( faceInfos ) );
        ...
        if humanInfo != None:  self.debug( "DBG: INF: abcdk.TrackHuman.update: humanInfo: %s" % str( humanInfo ) );
        if( humanInfo == None ):
            self.mem.raiseMicroEvent( "Tracking/NoFace",  [float(time.time()-self.timeBegin), None] );
        else:
            if( len(humanInfo) == 7 ):
                self.debug( "INF: abcdk.tracking.update: output face pos: %s" % humanInfo );
                face_id, face_x,face_y,face_h,face_pitch, face_yaw, face_roll = humanInfo;
                self.mem.raiseMicroEvent( "Tracking/FaceSeen",  [float(time.time()-self.timeBegin), humanInfo] );
            else:
                face_id, recognized_id, avg_age, avg_gender,face_x,face_y,face_h,face_pitch, face_yaw, face_roll = humanInfo;            
                
                if( recognized_id == "" ):
                    self.debug( "INF: abcdk.tracking.update: output age et genre: %s" % humanInfo );
                    self.mem.raiseMicroEvent( "Tracking/FaceAnalysed",  [float(time.time()-self.timeBegin),humanInfo ] );
                else:
                    self.debug( "INF: abcdk.tracking.update: output recognized '%s': %s" % (recognized_id, humanInfo) );
                    self.mem.raiseMicroEvent( "Tracking/FaceRecognised",  [float(time.time()-self.timeBegin), humanInfo ] );
                
            self.mem.raiseMicroEvent( "Tracking/FacePosition",  [float(time.time()-self.timeBegin), [face_id, face_x,face_y,face_h,face_pitch, face_yaw, face_roll] ] );
        
        retValue = None;
        if( self.bInUpdate ):
            self.debug( "INF: abcdk.TrackHuman.update: Skipping faces..." );
            return retValue;
        ...
        if( humanInfo == None or humanInfo == [] ):
            ...
            if( bUseSoundLocalisation and time.time() - self.timeLastHumanSeen > 3. and not self.bSoundLocalisationIsRunning ):
                self.debug("DBG: abcdk.TrackHuman.update: SoundLocalization - start" );
                ...
                if( bEnableRandomMoves and random.random() > 0.6 ): # return to zero a bit often
                    ...
                    self.debug( "DBG: abcdk.TrackHuman.update: random move to %s at speed %f" % (str( arPos ), rSpeed ) );
                bMoveHead = True;
                if( self.bTrackWithEyes ):
                    self.debug( "DBG: abcdk.TrackHuman.update: raz eyes (or random moves)..." );
                    self.eyesMover.moveLeft( arPos, rTime = 0.4, rDist = rDist, bWaitEnd = False );                    
                    bMoveHead = random.random() > 0.5;
                if( bMoveHead ):
                    self.debug( "DBG: abcdk.TrackHuman.update: raz head or random moves" );
                    self.debug( "**" * 50 );
                    if( self.idX != -1 ):
                        try:
                            self.motion.stop( self.idX );
                    ...
                    if( self.bTrackUsePitch ):
                        if( len(self.headChain) > 2 ):
                            arPos.append( arPos[1]*self.rPitchRatioThruJoint[0]);
                            arPos[1] *= self.rPitchRatioThruJoint[1];
                    ...
                    self.debug( "random moves task id: x: %d, y: %d" % ( self.idX, self.idY ) );
                    if( random.random() > 0.8 and self.bUseTrunk ):
                        self.debug( "DBG: abcdk.TrackHuman.update: raz trunk..." );
                        if( self.idTrunk != -1 ):
                            try:
                                self.motion.stop( self.idTrunk );
                            except BaseException, err:
        ...
        if( self.bSoundLocalisationIsRunning ):
            self.bSoundLocalisationIsRunning = False;            
            self.debug("DBG: abcdk.TrackHuman.update: SoundLocalization - stop" );           
            self.sl.unsubscribe( self.getName() );
        ...
        if( bTrackFace ):
            ...
            if( system.isOnRomeo() ):
                self.ledsDcm.setMouthColor( color = [0xFFFFFF,0x00FFF00,0xFFFFFF], rTime = 0.05 );
                self.ledsDcm.setMouthColor( color = [0x404040,0x404040,0x404040], rTime = 1. );
            
            aAngleToPutToHead = [rMoveX,rMoveY];
            bTooSmall = abs(aAngleToPutToHead[0]) < self.rThresholdToMove and abs(aAngleToPutToHead[1]) < self.rThresholdToMove;
            self.debug( "aAngleToPutToHead[0]: %s, aAngleToPutToHead[1]: %s" % ( aAngleToPutToHead[0],aAngleToPutToHead[1] ) )
            if( bTooSmall ):
                self.debug( "stop fix ? last:%s" % self.timeLastMoveToFix );
                if( time.time() - self.timeLastMoveToFix > 5. ):
                    self.debug( "stop fix! " * 20 );
            ...
            if( self.bTrackWithEyes ):
                ...
                if( not bTooSmall ):
                    ...
                    self.debug( "aCurPosEyes: %s" % aCurPos );
                    ...
                    #~ self.debug( "rect3: %f centred3: %f" % (rect[3], aCentred[3]) );
                    rDist = 0.31/face_h; # size in image to distance (approx on a medium sized face)
                    rTime = 0.2;                    

                    # inhibate some axis ?
                    for i in range(2):
                        if( self.abLastFrameFastMove[i] ):
                            # inhibate this axis
                            self.debug( "DBG: abcdk.TrackHuman.update: axis %d was inhibitate" % i );
                            self.abLastFrameFastMove[i] = False;
                            aAngleMoves[i] = aCurPos[i];
                            aAngleToPutToHead[i] = 0.;
                        else:
                            if(abs(aAngleToPutToHead[i]) > 0.1 ):
                                self.debug( "DBG: abcdk.TrackHuman.update: axis %d does a fast moves (inhibate next face position)" % i );
                                self.abLastFrameFastMove[i] = True;                            

                    # transfer eyes pos to head
                    if( 1 ):
                        for i in range(2):
                    ...
                    self.debug( "aAngleMoves: %s" % aAngleMoves );
                    self.stopAllMoves();
            ...
            bTooSmall = abs(aAngleToPutToHead[0]) < self.rThresholdToMove and abs(aAngleToPutToHead[1]) < self.rThresholdToMove;        
            if( not bTooSmall ):
                #yawTrue = self.motion.getAngles( "NeckYaw", True );
                #yawFalse = self.motion.getAngles( "NeckYaw", False );
                #self.debug( "yawTrue: %s, yawFalse: %s" % (yawTrue, yawFalse) );
                #arHeadPose = self.motion.getAngles( self.headChain, True );
                arHeadPose = self.motion.getAngles( self.headChain, False ); # 2015-07-10 Alma: bug in NeckYaw Sensor reading: so we read the order, as it's more precise... TODO: remove this line
                
                aAngleToPutToHead[0] = -aAngleToPutToHead[0];  # 2015-05-21 Alma: invertion du -aAngleToPutToHead en +aAngleToPutToHead, because it's inverted...
                self.debug( "***" * 42 );
                
                self.debug( "arHeadPose: %s" % arHeadPose );
                self.debug( "aAngleToPutToHead: %s" % aAngleToPutToHead );
                rNewPosX = arHeadPose[0]+aAngleToPutToHead[0]*self.rAmortiHeadX;
                self.debug( "rNewPosX: %s" % rNewPosX );
                if( self.idX != -1 ):
                    #~ self.debug( "stopping moves task id: x: %d" % ( self.idX ) );
                    try:
                        self.motion.stop( self.idX );
                    except BaseException, err:
                        self.debug( "DBG: abcdk.TrackHuman.update: normal error: %s" % str( err ) );
                        pass
                    self.idX = -1;                
                self.idX = self.motion.post.angleInterpolationWithSpeed( self.headChain[0], rNewPosX, self.rSpeedTrackX );
                self.debug( "rNewPosX: %s" % rNewPosX );
                
                # transfer head pos X to trunk
                if( abs(rNewPosX) > 0.3 and self.bUseTrunk ):
                    if( self.idTrunk != -1 ):
                        try: self.motion.stop( self.idTrunk );
                        except: pass
                        self.idTrunk =-1;
                    strJointName = "TrunkYaw";
                    rCurPose = self.motion.getAngles( strJointName, True )[0];
                    rNewPos = rCurPose-rNewPosX*0.7;
                    rMaxYaw = 0.5;
                    if( rNewPos > rMaxYaw ):
                        rNewPos = rMaxYaw;
                    if( rNewPos < -rMaxYaw ):
                        rNewPos = -rMaxYaw;                        
                    self.debug( "DBG: abcdk.TrackHuman.update: transfer head pos to trunk: rNewPosX: %s, rNewPos: %s" % (rNewPosX,rNewPos) );                        
                    self.idTrunk = self.motion.post.angleInterpolationWithSpeed( strJointName, rNewPos, 0.03 );
                        
                # track on Y
                if( abs(aAngleToPutToHead[1]) > 0.05 and self.bTrackUsePitch ):
                    if( self.idY != -1 ):
                        #~ self.debug( "stopping moves task id: y: %d" % ( self.idY ) );
                        try: 
                            self.motion.stop( self.idY );
                        except BaseException, err:
                            self.debug( "DBG: abcdk.TrackHuman.update: normal error: %s" % str( err ) );
                    ...
                    if( len(self.rPitchRatioThruJoint) > 1 ):
                        aPitchPosToSend = [rNewPosY*self.rPitchRatioThruJoint[0],rNewPosY*self.rPitchRatioThruJoint[1]]
                    else:
                        aPitchPosToSend = rNewPosY
                    self.idY = self.motion.post.angleInterpolationWithSpeed( self.headChain[1:], aPitchPosToSend, self.rSpeedTrackY );
                self.timeLastMoveToFix = time.time();
            else:
        ...
        if( pValue[1][2] > .5 ): # confidence (should be a parameter)
            if( self.id != -1 ):
                try:
                    ...
                    self.debug( "DBG: abcdk.TrackHuman.updateSound: normal error: %s" % str( err ) );
                    pass
            self.timeLastHeadMove = time.time();
            # triming
            rCurrentYaw, rCurrentPitch = self.motion.getAngles( "Head", True );
            if( pValue[1][0] + rCurrentYaw > + self.rTrackYawLimit ):
                self.debug( "DBG: abcdk.TrackHuman.updateSound: limit left");
                pValue[1][0] = self.rTrackYawLimit - rCurrentYaw;
            elif( pValue[1][0] + rCurrentYaw < - self.rTrackYawLimit ):
                self.debug( "DBG: abcdk.TrackHuman.updateSound: limit right");
                pValue[1][0] = - self.rTrackYawLimit - rCurrentYaw;
            aAngles = [ pValue[1][0],pValue[1][1] ]; # get position
            self.debug( "INF: abcdk.TrackHuman.updateSound: moving to %s" % str( aAngles ) );
            if( self.bTrackWithEyes ):
    ...
    def onFaceDetected( self, varname, value ):
        #self.debug( "DBG: abcdk.tracking.onFaceDetected: %s: %s" % (varname, value) );
        import traceback
        try:
    ...
    def onSoundLocalised( self, varname, value ):        
        self.debug( "DBG: abcdk.tracking.onSoundLocalised: %s: %s" % (varname, value) );
    ...
    def setDebugMode( self, bNewState = True ):
        ...
        if( not self.bUseNativeExtractor ):
            if( self.extractor == None ):
                self.debug("INF: abcdk.TrackHuman.setDebugMode: creating proxy to HaarExtractor..." );
                try:
                    self.extractor = loadFaceDetectorModule();
                except: pass
            if( self.extractor != None ):
