...
def getFileFirstLine( szFilename, bQuiet = False ):
  "read a file and return it's first line, or '' if not found, empty, ..."
  strBufferRead = getFileContents( szFilename, bQuiet = bQuiet );
...
def find_last_file( strPath, nNbrFileToReturn = 1 ):
    """    
    find the last alphabetical file in a folder
    - nNbrFileToReturn: return more than one file [...,penultimate, ultimate]
    return a list of filename, even if only one file is asked
    """    
    listFile = sorted(os.listdir(strPath) )
    # remove folder and concat name
    listFileOut = []
    for i in range(len(listFile)):
        strFullPath = strPath + "/" + listFile[i]
        if os.path.isfile(strFullPath) and not os.path.islink(strFullPath):
            listFileOut.append( strFullPath )
    return listFileOut[-nNbrFileToReturn:]
# find_last_file - end
#~ print find_last_file( "/tmp", 4 )
#~ exit()

def isFileOpenForWrite( strFilename ):
    """
    ...
    if( file ):
        ...
        return False
    return True
# isFileOpenForWrite - end    

global_moveFilesContinuously_bMustStop = False
def moveFilesContinuously( strPath, rBandwithMaxInKiloBytes, strHostname = "robotdata@10.0.161.123" ):
    print( "INF: moveFilesContinuously: beginning to move '%s' to '%s'" % (strPath, strHostname) )
    
    import system    
    global global_moveFilesContinuously_bMustStop
    
    global_moveFilesContinuously_bMustStop = False
    nBandwithMaxInKb = int(rBandwithMaxInKiloBytes * 8)
    strDstFolderName = system.getNickName()
    strDstPath = strHostname + ":~/" + strDstFolderName
    os.system( "ssh %s 'mkdir -p %s'" % (strHostname, strDstFolderName) )
    while not global_moveFilesContinuously_bMustStop:
        listFiles = sorted(os.listdir(strPath))
        for file in listFiles:
            if( global_moveFilesContinuously_bMustStop ):
                break
            strFullPath = strPath+"/"+file
            if( isFileOpenForWrite( strFullPath ) ):
                print( "INF: moveFilesContinuously: file '%s' is open..." % file )
            else:
                nSizeBefore = os.path.getsize( strFullPath )
                if( nSizeBefore > 0 ):
                    print( "INF: moveFilesContinuously: moving file '%s'" % file )
                    strCommand = "scp -q -r -l %s \"%s\" \"%s\"" % (nBandwithMaxInKb,strFullPath, strDstPath + "/" + file ) # todo check option VerifyHostKeyDNS
                    nRet = os.system( strCommand ) 
                    if( nRet != 0 ):
                        print( "WRN: moveFilesContinuously: '%s': scp error while copying: nRet: %s (command: '%s')" % (file, nRet,strCommand) )
                    else:
                        nSizeAfter = os.path.getsize( strFullPath )
                        if nSizeAfter != nSizeBefore:
                            print( "WRN: moveFilesContinuously: '%s' after scp, size has changed: %s then %s" % (file, nSizeBefore, nSizeAfter) )
                        else:
                            if( nRet == 0 ):
                                os.unlink( strFullPath )
        time.sleep( 10 )
    print( "INF: moveFilesContinuously: ended..." )
# moveFilesContinuously - end        

def moveFilesContinuously_stop():
    print( "INF: moveFilesContinuously_stop: ask for stop" )
    global global_moveFilesContinuously_bMustStop
    global_moveFilesContinuously_bMustStop = True
